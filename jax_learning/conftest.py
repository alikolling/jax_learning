...
For example, when running `python jax_learning/main.py algorithm=image_classifier`.
...
flowchart TD
datamodule_config[
    <a href="#jax_learning.conftest.datamodule_config">datamodule_config</a>
] -- 'datamodule=A' --> command_line_arguments
algorithm_config[
    <a href="#jax_learning.conftest.algorithm_config">algorithm_config</a>
] -- 'algorithm=B' --> command_line_arguments
command_line_overrides[
    <a href="#jax_learning.conftest.command_line_overrides">command_line_overrides</a>
] -- 'seed=123' --> command_line_arguments
command_line_arguments[
    <a href="#jax_learning.conftest.command_line_arguments">command_line_arguments</a>
] -- load configs for 'datamodule=A algorithm=B seed=123' --> dict_config
dict_config[
    <a href="#jax_learning.conftest.dict_config">dict_config</a>
] -- instantiate objects from configs --> config
config[
    <a href="#jax_learning.conftest.config">config</a>
] --> datamodule & algorithm
datamodule[
    <a href="#jax_learning.conftest.datamodule">datamodule</a>
] --> algorithm
algorithm[
    <a href="#jax_learning.conftest.algorithm">algorithm</a>
...
from jax_learning.experiment import instantiate_datamodule, instantiate_trainer
from jax_learning.main import (
    PROJECT_NAME,
    instantiate_algorithm,
    setup_logging,
)
from jax_learning.utils.env_vars import NUM_WORKERS, REPO_ROOTDIR
from jax_learning.utils.hydra_utils import resolve_dictconfig
from jax_learning.utils.testutils import (
    IN_GITHUB_CI,
    PARAM_WHEN_USED_MARK_NAME,
    default_marks_for_config_combinations,
    default_marks_for_config_name,
)
from jax_learning.utils.typing_utils import is_sequence_of

if typing.TYPE_CHECKING:
    from _pytest.mark.structures import ParameterSet

    # TODO: Might not be present if the Jax examples aren't included.
    from jax_learning.trainers.jax_trainer import JaxTrainer
...
def algorithm_config(request: pytest.FixtureRequest) -> str | None:
    ...
    included tests, for example as is done in [jax_learning.algorithms.image_classifier_test][].
    """
...
def command_line_arguments(
    ...
    indirectly by most tests using the [`jax_learning.utils.testutils.run_for_all_configs_of_type`][]
...
def dict_config(
    ...
    logger.info(
        ...
        + "\n"
        + "```\n"
        + ("python jax_learning/main.py " + " ".join(command_line_arguments) + "\n")
        + "```\n"
    )
...
def setup_with_overrides(
    ...
    def test_something(dict_config: omegaconf.DictConfig):
        ...
        assert dict_config["algorithm"]["_target_"] == "jax_learning.algorithm.image_classifier.ImageClassifier"
