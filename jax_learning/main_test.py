...
import jax_learning.configs
import jax_learning.experiment
import jax_learning.main
from jax_learning.algorithms.no_op import NoOp
from jax_learning.conftest import setup_with_overrides, skip_on_macOS_in_CI
from jax_learning.utils.env_vars import REPO_ROOTDIR
from jax_learning.utils.hydra_utils import resolve_dictconfig

logger = getLogger(__name__)

CONFIG_DIR = Path(jax_learning.configs.__file__).parent
...
def mock_train(monkeypatch: pytest.MonkeyPatch):
    mock_train_fn = Mock(spec=jax_learning.main.train, return_value=(None, None))
    monkeypatch.setattr(jax_learning.main, jax_learning.main.train.__name__, mock_train_fn)
    return mock_train_fn


@pytest.fixture
def mock_evaluate(monkeypatch: pytest.MonkeyPatch):
    mock_eval = Mock(spec=jax_learning.experiment.evaluate, return_value=("fake", 0.0, {}))
    monkeypatch.setattr(
        jax_learning.main,
        jax_learning.experiment.evaluate.__name__,
        mock_eval,
    )
    return mock_eval
...
def test_can_load_experiment_configs(
    ...
    if dict_config["hydra"]["mode"] == RunMode.MULTIRUN:
        ...
        results = jax_learning.main.main(dict_config)
...
def test_can_run_experiment(
    ...
    command_line_args = ["jax_learning/main.py"] + list(command_line_overrides) + [f"name={name}"]
...
def test_help_string(file_regression: FileRegressionFixture) -> None:
    help_string = subprocess.run(
        ...
        shlex.split("python jax_learning/main.py seed=123 --help"),
        text=True,
        capture_output=True,
    ).stdout
...
def test_run_auto_schema_via_cli_without_errors():
    ...
    # assuming we're at the jax_learning root directory.
